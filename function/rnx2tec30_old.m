...
for st = 1:size(rnxstat,1)
    ...
    %         try
    %             [~, nav_data] = nav2data(char(R_path + navfile(nf).name));
    ...
    %     end
    % else
    %     [~, nav_data] = nav2data(char(R_path + navfile_name));
    % end
    %% find ref position
        ...
        for pind = 1:length(prn_sys)
            ...
            try
                ...
                if sum(~isnan(P1+P2))<length(gpst)/2 || sum(~isnan(L1+L2))<length(gpst)/2
                    disp(["SYS: " + sys_name + " PRN# ... " + num2str(prn) + " - Not enough pseudoranges pair:" + char(pairing(best_pair,1)) + "-" + char(pairing(best_pair,2))])
                    continue
                end
                flag = 1;
            catch
                % disp(sys_name + " system has enogth data of this pair:" + char(pairing(pr,1)) + "-" + char(pairing(pr,2)))
                continue
            end
            if flag==0
    ...
    if ~exist(S_path + year + '\' + doy, "dir"); mkdir(S_path + year + '\' + doy); end
