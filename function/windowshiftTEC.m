...
for LP = 1:size(STECl,2)
    ...
    for SW = 1:length(wind)
        if SW == 1
            ...
            *diag(mean(STECp(1:wind(SW),LP)...
            -STECl(1:wind(SW),LP),"omitnan"));
        
        else
            STEC_adj(wind(SW-1):wind(SW),LP)=STECl(wind(SW-1):wind(SW),LP) ...
            + ones(size(STECl(wind(SW-1):wind(SW),LP)))...
            *diag(mean(STECp(wind(SW-1):wind(SW),LP)...
            -STECl(wind(SW-1):wind(SW),LP),"omitnan"));
        end     
    end
    clear wind
end
