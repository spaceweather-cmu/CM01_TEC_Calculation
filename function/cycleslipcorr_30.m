function [STEC_new] = cycleslipcorr_30(STECl,Elevation,Time,PRNall)
%% ==== Descriptions ====
% For correct the Cycle slip effect
% Writed by Somkit Sophan, Napat; Date: Jan, 2020
% Updated - Napat; date: Sep, 2020 - add the data missing correction
%         - Napat; date: Nov, 2024 - improve detection   
...
for PRN = PRNall
    ...
    gap_bound     = mean(diff(STEC),"omitnan"); % estimate gap bound to define as cycle-slip
    if gap_bound <= 0.04*(min(diff(T))) % min gap
        gap_bound = 0.04*(min(diff(T)));
    end
    gap_time   = 2*60*60; % 2 hours
    dif_time   = diff(T);
    time_gap   = find(dif_time>=gap_time); 
    ...
    %% 2. STEC has time gap (3 hours)  
        ...
        for iii = 1:length(time_gap)-1
            ...
            elev_g = elev(time_gap(iii)+1:time_gap(iii+1));                 % group elev
