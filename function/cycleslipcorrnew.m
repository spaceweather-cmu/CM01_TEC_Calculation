function [STEC_new] = cycleslipcorrnew(STECl,Elevation,Time,PRNall)
%% ==== Descriptions ====
...
for PRN = PRNall
    ...
    if gap_bound <= 0.5 % min gap
        gap_bound = 0.5*(min(diff(T)));
    end
    if gap_bound >= 1 % max gap
        gap_bound = 1*(min(diff(T)));
    end
    gap_time   = 1*60*60; % 1 hours
    dif_time   = diff(T);
